					...
					if(resultat.length == 0)
					{
						...
						.on('end', function() {
							...
							con.query(sql, function (err, result) {
								id_video_create = result.insertId;
								pathVideo = folderVideoCreate + '/' + textVideo.toUpperCase() + '.mp4'
								videoFinalName = textVideo.toUpperCase() + '.mp4"'
/*								createScreenShot(pathVideo, videoFinalName, id_video_create).then(function(leResultat) {
...
/*		function createScreenShot(pathVideo, videoFinalName, id_video_create)
		{
			//return new Promise((resolve, reject) => {
				var proc = fluent_ffmpeg(pathVideo)
				.takeScreenshots({
					count: 1,
		      timemarks: [ '0.5' ], // number of seconds
		      filename: '%f'
		  }, '/home/jim044/Projets/SpeechVIP/screenshot/', function(err, filenames) {
