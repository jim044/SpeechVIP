...
module.exports = function(app) {
							 		...
							 		changeChannel.on('close', (code) => {
							 			...
							 			segment.on('close', (code) => {
							 				fs.chmod(audiofile192BitrateMono, '0777', function(err){
							 					...
							 					{
							 						getAudioDurationInSeconds(audiofile192BitrateMono).then((audioDuration) => {
							 							...
							 							//con.query(sql, function (err, result) {
							 								...
							 								cutAudioByWord(audiofile192BitrateMonoCut, Math.ceil(numberCutAudio)).then(function(leResultat) {
							 								})
							 							//});
							 						})	
							 					}
		...
		return new Promise(function(resolution, rejet)
		{
			...
			for (var i=0; i<3; i++) {
				...
				//sleep.sleep(1);
				 ...
				 recognitionAudio(nomFichierWithNumber, i).then(function(leResultat) {
				 })
	...
	{
		return new Promise(function(resolveAgain, RejectAgain)
		{
		...
		.then(data => {
			...
			response.results.forEach(result => {
		        ...
		        var finDuDecoupage = (i * 10) + parseFloat(endSecs) + 0.2
		        //console.log('startSecs : ' + startSecs)
		        //console.log('Debut decoupage : ' + debutDuDecoupage)
		        //console.log('i ' + i)
		        console.log('_NUmber Cut_' + numberCut);
		        cutVideo(nameOfFile, folderName, fileNameVideo, debutDuDecoupage, finDuDecoupage, numberCut).then(function(leResultat) {
		        })
		        numberCut += 1;
		    });
			});
	...
	var cutVideo = function(nameOfFile, folderName, videoPath, startTime, endTime, numberToCut)
	{
		return new Promise(function(resolve, reject)
		{
			//console.log('_NUmber CUt _' + numberToCut);
			...
			.on('end', function(err) {   
				if(!err)
				{
					console.log('conversion Done : ' + numberToCut);
					resolve('conversion Done');
				}                 

			})
			.on('error', function(err){
				console.log('error: video cut : ' + numberToCut);
				//sleep.sleep(2);
				cutVideo(nameOfFile, folderName, videoPath, startTime, endTime, numberToCut).then(function(leResultat) {
		        })
			}).run();
		})
	}
