...
module.exports = function(app) {
						...
						con.connect(function(err) {
							con.query('SELECT * FROM video WHERE nom="' + filevideo + '"', async function (err, result, fields) {
								if(result.length == 0)
								{
									...
									con.connect(function(err) {
										con.query('SELECT * FROM personne WHERE PrenomNom="' + req.query.personne + '"', async function (err, result, fields) {
											...
											con.connect(function(err) {
												con.query('SELECT * FROM langue WHERE languageCode="' + codelangue + '"', async function (err, result, fields) {
													...
													console.log('INSERT INTO video (id_personne, id_language, nom, duree) VALUES (' + idPersonne + ',' + idLangue + ',' + '"' + filevideo + '",' + '"' + dureevideo + '")')
	...
	const getCutVideo = async (nombreElementACouper, WordsArray, fileName, getFileNameSearch, getFilePathCut, nameOfFile, debutDuDecoupage, finDuDecoupage, i) => {
		...
		/*function createScreenShot(pathVideo)
		{
			var proc = new ffmpeg(pathVideo)
		  	.takeScreenshots({
		      count: 1,
		      timemarks: [ '600' ] // number of seconds
		    }, '/home/jim044/Projets/SpeechVIP/screenshot/', function(err) {
