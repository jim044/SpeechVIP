	...
	var recognitionAudio = async function(nomFichierWithNumber, i, nombreElementACouper)
	{
		...
		.then(data => {
			...
			/*response.results.forEach(async (result) => {*/
				for (var j = 0;j<response.results[0].alternatives[0].words.length;j++)
				{
					var wordInfo = response.results[0].alternatives[0].words[j];
					...
					if (wordInfo.confidence > 0.9)
					{
						...
						//response.results[0].alternatives[0].words.forEach(async (wordInfo) => {
...
var cutVideo = function(sentenceTranscrib, nameOfFile, folderName, videoPath, startTime, endTime, numberToCut)
{
	/*return new Promise( function(resolve, reject)
	{*/
		...
		endTimeOffset = parseFloat(endTimeOffset)
			...
			/*if(sentenceTranscrib.length >= 10)
			{
			...
			{		
				startTimeOffset = parseFloat(startTimeOffset)
				endTimeOffset = parseFloat(endTimeOffset) - (parseFloat(duration) * 0.12)
				console.log("extra long")
