...
module.exports = function(app) {
	...
	var recognitionAudio = async function(nomFichierWithNumber, i, nombreElementACouper)
	{
		return new Promise(async function(resolveAgain, RejectAgain)
		{
			...
			.then(data => {
				...
				console.log("response.results.length " + response.results[0].alternatives[0].words.length);
				var WordsArray = response.results[0].alternatives[0].words;

				getCutVideo(nombreElementACouper, numberCut, WordsArray, fileName, getFileNameSearch, getFilePathCut, nameOfFile, debutDuDecoupage, finDuDecoupage, sentence, WordSentenceAdd, retourCutvideo, i).then(() => {
					console.log('done');
					resolveAgain('Fin');
				})
	...
	const cutVideo = function(sentenceTranscrib, nameOfFile, folderName, videoPath, startTime, endTime, numberToCut) 
	{
	...
	const getCutVideo = async (nombreElementACouper, numberCut, WordsArray, fileName, getFileNameSearch, getFilePathCut, nameOfFile, debutDuDecoupage, finDuDecoupage, sentence, WordSentenceAdd, retourCutvideo, i) => {
		var j = 0;
		for ( const item of WordsArray)
		{
			...
			if (wordInfo.confidence > 0.9)
			{
				...
				if( j % 3 === 0)
				{
	        		...
	        		retourCutvideo = cutVideo(sentence, nameOfFile, folderName, fileNameVideo, debutDuDecoupage, finDuDecoupage, numberCut) //.then(function(leResultat) {
	        			if (retourCutvideo ==  sentence)
	        			{
