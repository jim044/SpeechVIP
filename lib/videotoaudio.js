...
module.exports = function(app) {
	app.get('/videotoaudio', function(req, res) {
		...
		{
			try {
				console.log(pathvideoAttente + '/' + filevideo)
			...
			response.results.forEach(async (result) => {
				for (var j = 0;j<result.alternatives[0].words.length;j++)
				{
					...
					if (wordInfo.confidence > 0.9)
					{
				        ...
				        // var testAddition = wordInfo.startTime.seconds + ( wordInfo.endTime.nanos / 100000000)
				        console.log('startSecs : ' + startSecs)
				        console.log('endSecs : ' + endSecs)
				        // console.log('testAddition : '  +  `${wordInfo.endTime.seconds}` + `.` + wordInfo.endTime.nanos / 100000000)
				        // console.log('wordInfo.startTime.nanos : ' + wordInfo.startTime.nanos)
				        // console.log('wordInfo.endTime.nanos : ' + wordInfo.endTime.nanos)
	...
	var cutVideo = function(wordTranscrib, nameOfFile, folderName, videoPath, startTime, endTime, numberToCut)
	{
		return new Promise( function(resolve, reject)
		{
			var startTimeOffset = startTime;
			var endTimeOffset = endTime;

			if(wordTranscrib.length <= 4)
			{
				// startTimeOffset = startTime + 0.1;
				// endTimeOffset = endTime - 0.20;
			}
			else if(wordTranscrib.length > 4 && wordTranscrib.length < 6)
			{
				// console.log(wordTranscrib)
				// startTimeOffset = startTime - 0.10;
				// endTimeOffset = endTime - 0.16;
			}
			else
			{
				// console.log(wordTranscrib)
				// startTimeOffset = startTime + 0.2;
				// endTimeOffset = endTime - 0.3;
			}
			...
			.setStartTime(parseFloat(startTimeOffset))
