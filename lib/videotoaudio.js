...
module.exports = function(app) {
	...
	var rename = function(filevideoToRename, filevideo)
	{
								...
								if(result.length == 0)
								{
									...
									// con.query(sql, function (err, result) {
										...
										extractAudioFromVideo(filevideo);
										 ...
										 await extractAudioFromVideo(filevideo);//.then(function(resultatExtractAudio) {
	...
	var cutAudioByWord = function(fileAudioName, numberOfCut)
	{
		return new Promise(async function(resolution, rejet)
		{
			...
			for (var i=0; i<numberOfCut; i++) {
				...
				//console.log('Nom fichier a supprimer : ' + nomFichierASupprimer)
