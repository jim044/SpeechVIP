...
module.exports = function(app) {
	...
	app.post('/addLanguage', function(req, res) {
		...
		var codelanguage = req.body.codelanguage;
		...
		con.connect(function(err) {
			con.query('SELECT * FROM langue WHERE languageCode="' + codelanguage + '"', async function (err, result, fields) {
				if(result.length == 0)
				{
					if (err) console.log(err);
					var sql = 'INSERT INTO langue (language, languageCode) VALUES (' + '"' + language + '",' + '"' + codelanguage + '")';
					con.query(sql, function (err, result) {
						if (err) console.log(err);
						});
					res.send("Finish")
	...
	app.post('/convertVideo', function(req, res) {
		...
		res.redirect('/videotoaudio?filename=' + video + '&language=' + language + '&personne=' + personne);
		//res.send("Finish")
	})
